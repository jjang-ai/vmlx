...
"""Native MTP (Multi-Token Prediction) runtime adapters for mlx-lm.

The adapter contract is intentionally small: a model gains an extra ``mtp``
module, a callable ``mtp_forward`` method, and an enhanced ``__call__`` that
can return hidden states alongside logits. vMLX owns the draft/verify loop on
top of those hooks.
...
def apply_mlx_lm_mtp_patch() -> bool:
    ...
    if not cache_rollback.apply():
        return False
    try:
        qwen_ok = qwen35_model.apply()
    except Exception as exc:
        qwen_ok = False
        logger.debug("Qwen3.5/3.6 MTP patch raised: %s", exc)
    if not qwen_ok:
    ...
    try:
        dsv4_ok = deepseek_v4_model.apply()
    except Exception as exc:
        dsv4_ok = False
    ...
    _PATCHED = True
    logger.info("mlx-lm MTP adapters applied")
    return True
