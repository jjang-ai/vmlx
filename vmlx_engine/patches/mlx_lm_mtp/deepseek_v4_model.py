...
"""vMLX runtime adapter for DeepSeek-V4-Flash native MTP.

The DeepSeek-V4 MTP head architecture is heavier than Qwen's: each
``MTPBlock`` wraps a full
...
def apply() -> bool:
    """Apply DeepSeek-V4 model-side adapters when the base patch is active."""
    global _PATCHED
    if _PATCHED:
        return True
    ...
    logger.info("DeepSeek-V4 MTP model adapter applied")
    return True
...
def _patch_model_args(dsv4: Any) -> None:
    ...
    vMLX widens the compress_ratios list from ``num_hidden_layers`` to
    ``num_hidden_layers + num_nextn_predict_layers`` (default 0, no
    compression). The original ``__post_init__`` raises if the list length
    does not match num_hidden_layers, so the adapter extends the list before
    that check by wrapping ``from_dict``.
    """
...
# and make sanitize aware of DeepSeek MTP weight remapping.
# ---------------------------------------------------------------------------
...
def _patch_model(dsv4: Any) -> None:
    ...
    def make_mtp_cache(self):
        """Build per-MTP-block caches with the same layout choices as the backbone.

        Current DeepSeek MTP layers default to ``compress_ratio=0`` so the
        common case is a plain RotatingKVCache, but vMLX honors the same
        SparseCompressedAttention / CacheList layout as the backbone for any
        config that assigns a non-zero compress_ratio to MTP layers.
        """
    ...
    def mtp_forward(self, h, input_ids, cache=None):
        ...
        Each MTP block fuses ``h`` with the embedded ``input_ids`` through
        its ``e_proj``/``h_proj`` projection and passes the result through a
        ``DeepseekV4Block``. The last block's ``hc_head`` collapses the
        Hyper-head dimension before ``norm`` and the shared ``lm_head``
        produce logits.
        """
        if cache is None:
    ...
    def sanitize(self, weights: Dict[str, Any]) -> Dict[str, Any]:
        """Combined oMLX-base + vMLX MTP sanitize.

        oMLX's stock sanitize strips ``mtp.*`` and remaps the FP4 expert
        weights / Hyper-head names. vMLX keeps ``mtp.*`` when an MTP head is
        present, nests block-internal weights under ``.block.``, and stacks
        routed expert weights for MTP layers as well as backbone layers.
        """
        n_layers = self.args.num_hidden_layers
        has_mtp = hasattr(self, "mtp")
        has_mtp_weights = any(k.startswith("mtp.") for k in weights)
        # Disable MTP module if weights are absent, for example when a
        # quantized checkpoint stripped them.
        ...
        # Block-internal weight key remapping. Adds ``mtp.*`` →
