...
# Apache-2.0; native extension attribution is in
# native_extensions/qsa_kernels/mtplx_qsa_kernels/NOTICE. MTPLX license
# and NOTICE are retained in vmlx_engine/metal/third_party/mtplx_gdn/.
"""Direct Steel sparse-GQA attention consumer for Qwen4Exp QSA prefill.

The native sparse QK stage reads selected blocks directly from the cache,
without constructing a gathered KV tensor. It scales queries in the input
dtype, uses Steel MMA for QK, and writes scores at absolute token positions
in that dtype. The adapter uses MLX's precise softmax and preserves the
stock probability-value matrix layout. Dense score/probability storage is
intentional: compacting it changes rounding through the full quantized model.
...
to ignore ``block_valid``.  Set ``VMLX_QWEN4_PREFILL_DIRECT_VALIDATE=1`` to
re-prove the invariant per call at the cost of a host synchronization.

Environment
-----------
``VMLX_QWEN4_PREFILL_DIRECT``
    ``1`` opts into this consumer. Unset defaults to off. The native module
    must also be loaded, ABI-compatible, and pipeline-ready.
``VMLX_QWEN4_PREFILL_DIRECT_VALIDATE``
...
def _verify_abi(ext, import_error):
    ...
    try:
        ...
        return None, _detach(exc)
    if not callable(getattr(ext, "qwen4_qsa_sparse_gqa_dense_scores", None)):
        error = RuntimeError("Native QSA extension lacks materialized-score API; rebuild required")
        logger.warning("%s: %s", __name__, error)
        return None, error
    return ext, import_error
...
def qsa_prefill_direct_enabled() -> bool:
    """Explicit VMLX opt-in; the native module gate is separate."""

    return _env_flag("VMLX_QWEN4_PREFILL_DIRECT", default=False)


def _validate_enabled() -> bool:
    return _env_flag("VMLX_QWEN4_PREFILL_DIRECT_VALIDATE", default=False)
...
def qsa_prefill_direct_unsupported_reason(
    ...
    *,
    ...
    if rows < 16:
        return "stock-order prefill requires at least 16 query rows"
...
def qsa_prefill_direct(
    ...
    *,
    ...
    scores = _EXT.qwen4_qsa_sparse_gqa_dense_scores(
        queries, keys, selected, float(scale), int(pos_start)
    )
    probabilities = mx.softmax(scores, axis=-1, precise=True)
    out = (probabilities @ values[:, :, None]).reshape(queries.shape)
...
def _prove_all_pipelines() -> bool:
    ...
    return all(_prove_pipeline(dtype=dtype) for dtype in _SUPPORTED_DTYPES)
