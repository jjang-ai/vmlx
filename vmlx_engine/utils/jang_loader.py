...
def _vlm_quant_module_path_candidates(module_path: str, model_type: str = "") -> set[str]:
    ...
    if str(model_type or "").lower() == "dots3_note":
        ...
        # while runtime text modules live under language_model.model.*.
        if module_path.startswith("language_model.model."):
            candidates.add(
                module_path.replace("language_model.model.", "model.", 1)
            )
...
def _ensure_jang_family_runtime_supported(path: Path, config: dict | None) -> None:
    ...
    if "hy_v3" in model_types:
        _import_required_jang_runtime(
            ...
            family="Hy3/hy_v3",
            remediation=(
                "Install jang>=2.5.34 or bundle the current local "
        ...
        _import_required_jang_runtime(
            ...
            remediation=(
                ...
                "use jang>=2.5.34 and do not strip the hy3 runtime from the bundle"
            ),
        )
...
def _patch_turboquant_make_cache(model, jang_cfg: dict, model_config: dict):
    ...
    if _tq_auto_generated:
        # ASYMMETRY GUARD (shared with the capability-only load path in
        # tokenizer._apply_turboquant_to_model — see the helper's docstring
        # for the measured cold/warm evidence). Engine-invented policies only:
        # a bundle-calibrated turboquant block is the author's intent.
        _tq_cfg = disable_auto_storage_tq_if_asymmetric(_tq_cfg)
