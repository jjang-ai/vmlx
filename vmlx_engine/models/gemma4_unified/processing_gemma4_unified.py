...
class Gemma4UnifiedProcessor(Gemma4Processor):
    ...
    def process(
        ...
        **kwargs,
        ...
        """Expose the same audio-capable signature path used by mlx-vlm utils.

        mlx-vlm's ``process_inputs`` prefers ``processor.process`` over
        ``processor.__call__`` when present. ``ProcessorMixin.process`` does not
        advertise Gemma4 Unified's audio argument, so audio requests were
        rejected before this processor's audio-capable ``__call__`` path ran.
        """
