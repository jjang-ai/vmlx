...
| `--cache-memory-percent` | Fraction of available RAM (default: 0.30) |
...
## Stability and Concurrency Controls

The scheduling engines (`LLM` and `MLLM`) use several mechanisms to guarantee stability in highly concurrent environments:

- **Ghost Request Detection**: Tracks orphaned outputs generated by the batch generator that have lost their HTTP consumer. Aborts these safely (checked every 50 steps) to prevent a permanent GPU spin-loop that blocks the event loop.
- **Cache Corruption Recovery**: If a `TypeError` or `IndexError` occurs inside the MLX generator due to stale offset states, the scheduler automatically dumps the batch context, clears the active memory cache (`mx.clear_memory_cache()`), and seamlessly respawns the `wait` queue requests without disconnecting the client (up to 2 times).
- **Engine Loop Isolation**: In the `MLLM` architecture, the heavy, synchronous vision-encoding computations runs within `asyncio.to_thread()`, isolated from the asynchronous HTTP connection loop. We govern interactions using thread-safe structures (`threading.RLock`, `asyncio.Queue`) so that rapid API connects/disconnects never corrupt the scheduler state.
- **Dynamic Memory Rejection**: Before scheduling waiting sequences into the KV batch, the scheduler computes whether the KV arrays merger will exceed available physical RAM and falls back to un-cached generation safely to avert `OOM` crashes.

> [!WARNING]
> **LLM Single Sampler Constraint**: The underlying `mlx-lm.BatchGenerator` forces the entire batch to share the sampling parameters of the *first* request. If you send mixed requests (e.g. `temperature=0.0` and `temperature=1.0`), all requests will momentarily decode with the *first* request's parameters! Wait to send identical sampling batches if determinism is essential. The `MLLMScheduler` does *not* have this limitation because it manages its own batch structure.
