...
class DeepseekV3Attention(nn.Module):
    ...
    def __call__(
        ...
        if L == 1:
            ...
            # in logit magnitude per token vs the prefill path, causing
            # decode-time repetition loops. Cast just the SDPA inputs to
            # float32 so the attention kernel accumulates in higher precision.
            # L=1 tensors are small so the extra cost is negligible (~1-3% vs
            # bf16 SDPA). Cast output back to bf16 before the unembed_out.
            # Mirror of research/deepseek_v32_patched.py fix applied for
            # GLM-5.1 / DSV3.2 on 2026-04-11. Same MLA absorb pattern, same
            # fix applies to deepseek_v3 (used by kimi_k25 wrapper).
