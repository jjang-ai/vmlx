...
describe('folder-first launch contract', () => {
  ...
  it('keeps selection separate from launch and clears per-folder overrides', () => {
    ...
    expect(source).toContain('const [showCustom, setShowCustom] = useState(true)')
    ...
    const browse = source.slice(source.indexOf('const handleBrowse'), source.indexOf('// Get allowed quantize'))
