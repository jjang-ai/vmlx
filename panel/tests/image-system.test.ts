...
describe('image IPC backend request disconnect guards', () => {
  it('routes local image server request writes through EPIPE-aware helpers', () => {
    ...
    expect(imageIpcSource).toContain('const wrappedDisconnects = [')
    expect(imageIpcSource).toContain('(err as any)?.reason')
    expect(imageIpcSource).toContain('(err as any)?.error')
    expect(imageIpcSource).toContain('(err as any)?.detail')
    expect(imageIpcSource).toContain('wrappedDisconnects.some((nested) => isExpectedImageServerDisconnectError(nested))')
