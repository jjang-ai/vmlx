...
export function parseContentArray(
  ...
  try {
    ...
    if (
      ...
      parsed.length > 0 &&
      parsed.every((p) => p && typeof p.type === "string")
    )
      return parsed;
  } catch {
    /* not JSON */
  ...
  if (metrics.ppSpeed) {
    ...
    items.push({
      label: isUncachedTail
        ...
        : `${metrics.ppSpeed} pp/s to first token`,
  ...
  switch (name) {
    ...
    case "read_file":
      return {
        label: "Read",
        context: args.path
          ? `${args.path}${args.offset ? `:${args.offset}` : ""}`
          : "",
      };
    case "write_file":
    ...
    case "insert_text":
      return {
        label: "Insert",
        context: args.path ? `${args.path}:${args.line || ""}` : "",
      };
    case "replace_lines":
      return {
        label: "Replace Lines",
    ...
    case "search_files":
      return {
        label: "Search",
        context: `"${truncateStr(args.pattern, 40)}"${args.path && args.path !== "." ? ` in ${args.path}` : ""}`,
      };
    case "find_files":
      return {
        label: "Find",
    ...
    case "ddg_search":
      return {
        label: "Search Web",
        context: `"${truncateStr(args.query, 50)}"`,
      };
    case "fetch_url":
