...
function getModelType(s: SessionSummary): string {
  try {
    ...
    if (cfg.modelType === "image")
      return cfg.imageMode === "edit" ? "image-edit" : "image-gen";
...
export function ApiDashboard() {
  ...
  const runningSessions = useMemo(
    () =>
      (sessions as SessionSummary[]).filter(
        (s) => s.status === "running" || s.status === "standby",
      ),
    [sessions],
  );
        ...
        <div className="p-4 rounded-lg border border-border bg-card space-y-3">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-2 text-xs">
              ...
              />
              {portError && (
                <span className="text-[10px] text-red-400">{portError}</span>
              )}
            </div>
            ...
            <div className="border border-border rounded-lg divide-y divide-border overflow-hidden">
              {runningSessions.map((s) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${isSleeping ? "bg-blue-400" : "bg-green-500"}`}
                    />
