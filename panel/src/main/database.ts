...
class DatabaseManager {
  ...
  constructor() {
    ...
    try {
      ...
      console.log(
        `[DB] Fresh database created. Corrupt file saved to: ${backupPath}`,
      );
    }
  }
  ...
  private initialize(): void {
    ...
    const runMigrations = this.db.transaction(() => {
      ...
      const chatColumns = this.db.pragma("table_info(chats)") as {
        name: string;
      }[];
      ...
      const msgColumns = this.db.pragma("table_info(messages)") as {
        name: string;
      }[];
      ...
      const overrideColumns = this.db.pragma("table_info(chat_overrides)") as {
        name: string;
      }[];
      ...
      if (!overrideColumns.find((c) => c.name === "max_tool_iterations")) {
        this.db.exec(
          "ALTER TABLE chat_overrides ADD COLUMN max_tool_iterations INTEGER",
        );
      }
      if (!overrideColumns.find((c) => c.name === "builtin_tools_enabled")) {
        this.db.exec(
          "ALTER TABLE chat_overrides ADD COLUMN builtin_tools_enabled INTEGER DEFAULT 0",
        );
      }
      if (!overrideColumns.find((c) => c.name === "working_directory")) {
        this.db.exec(
          "ALTER TABLE chat_overrides ADD COLUMN working_directory TEXT",
        );
      }
      ...
      if (!overrideColumns.find((c) => c.name === "hide_tool_status")) {
        this.db.exec(
          "ALTER TABLE chat_overrides ADD COLUMN hide_tool_status INTEGER DEFAULT 0",
        );
      }
      if (!overrideColumns.find((c) => c.name === "web_search_enabled")) {
        this.db.exec(
          "ALTER TABLE chat_overrides ADD COLUMN web_search_enabled INTEGER DEFAULT 1",
        );
      }
      if (!overrideColumns.find((c) => c.name === "fetch_url_enabled")) {
        this.db.exec(
          "ALTER TABLE chat_overrides ADD COLUMN fetch_url_enabled INTEGER DEFAULT 1",
        );
      }
      if (!overrideColumns.find((c) => c.name === "file_tools_enabled")) {
        this.db.exec(
          "ALTER TABLE chat_overrides ADD COLUMN file_tools_enabled INTEGER DEFAULT 1",
        );
      }
      if (!overrideColumns.find((c) => c.name === "search_tools_enabled")) {
        this.db.exec(
          "ALTER TABLE chat_overrides ADD COLUMN search_tools_enabled INTEGER DEFAULT 1",
        );
      }
      ...
      if (!overrideColumns.find((c) => c.name === "reasoning_effort")) {
        this.db.exec(
          "ALTER TABLE chat_overrides ADD COLUMN reasoning_effort TEXT",
        );
      }
      if (!overrideColumns.find((c) => c.name === "brave_search_enabled")) {
        this.db.exec(
          "ALTER TABLE chat_overrides ADD COLUMN brave_search_enabled INTEGER DEFAULT 0",
        );
      }
      if (!overrideColumns.find((c) => c.name === "tool_result_max_chars")) {
        this.db.exec(
          "ALTER TABLE chat_overrides ADD COLUMN tool_result_max_chars INTEGER",
        );
      }
      if (!overrideColumns.find((c) => c.name === "git_enabled")) {
        this.db.exec(
          "ALTER TABLE chat_overrides ADD COLUMN git_enabled INTEGER DEFAULT 1",
        );
      }
      ...
      if (gitCol && gitCol.dflt_value === null) {
        this.db.exec(
          "UPDATE chat_overrides SET git_enabled = 1 WHERE git_enabled IS NULL",
        );
      }
      const utilCol = tableInfo.find((c) => c.name === "utility_tools_enabled");
      if (utilCol && utilCol.dflt_value === null) {
        this.db.exec(
          "UPDATE chat_overrides SET utility_tools_enabled = 1 WHERE utility_tools_enabled IS NULL",
        );
      }
      ...
      const sessionColumns = this.db.pragma("table_info(sessions)") as {
        name: string;
      }[];
      if (!sessionColumns.find((c) => c.name === "type")) {
        this.db.exec(
          "ALTER TABLE sessions ADD COLUMN type TEXT NOT NULL DEFAULT 'local'",
        );
      }
      ...
      if (!sessionColumns.find((c) => c.name === "remote_organization")) {
        this.db.exec(
          "ALTER TABLE sessions ADD COLUMN remote_organization TEXT",
        );
      }
      ...
      try {
        const indexInfo = this.db.pragma("index_list(sessions)") as {
          name: string;
          unique: number;
        }[];
        const hasUniquePort = indexInfo.some(
          (idx) =>
            idx.unique === 1 &&
            (
              this.db.pragma(`index_info(${idx.name})`) as { name: string }[]
            ).some((col) => col.name === "port"),
        );
        if (!hasUniquePort) {
        ...
        `);
          this.db.exec(
            "CREATE UNIQUE INDEX IF NOT EXISTS idx_sessions_port_unique ON sessions(port)",
          );
    ...
    `);
      ...
      const templateCount = (
        this.db
          .prepare(
            "SELECT COUNT(*) as cnt FROM prompt_templates WHERE is_builtin = 1",
          )
          .get() as any
      ).cnt;
      if (templateCount === 0) {
        const builtins = [
          {
            ...
            content:
              "You are an expert software engineer. Write clean, efficient, well-documented code. Explain your reasoning. When debugging, think step-by-step. Prefer simple solutions.",
          },
          {
            id: "builtin-writer",
            ...
            category: "creative",
            content:
              "You are a skilled creative writer. Write vivid, engaging prose. Vary sentence structure and length. Show, don't tell. Use sensory details and strong verbs.",
          },
          {
            id: "builtin-analyst",
            name: "Data Analyst",
            category: "analysis",
            content:
              "You are a data analyst. Provide clear, data-driven insights. Use precise numbers. Structure your analysis with sections. Highlight key findings and actionable recommendations.",
          },
          {
            id: "builtin-tutor",
            name: "Patient Tutor",
            category: "education",
            content:
              "You are a patient, encouraging tutor. Explain concepts from first principles. Use analogies and examples. Check understanding. Adapt your explanations to the student's level.",
          },
          {
            id: "builtin-concise",
            ...
            category: "general",
            content:
              "Be extremely concise. Answer in as few words as possible while remaining accurate and helpful. No unnecessary elaboration. Bullet points preferred.",
          },
          {
            id: "builtin-socratic",
            name: "Socratic Guide",
            category: "education",
            content:
              "Guide through questions rather than direct answers. Ask thought-provoking questions that lead to understanding. Help the user discover the answer themselves.",
          },
          {
            id: "builtin-reviewer",
            name: "Code Reviewer",
            category: "development",
            content:
              "You are a thorough code reviewer. Check for bugs, security issues, performance problems, and style. Be specific about line numbers. Suggest concrete improvements. Rate severity.",
          },
        ];
        const ins = this.db.prepare(
          "INSERT INTO prompt_templates (id, name, content, category, is_builtin, created_at) VALUES (?, ?, ?, ?, 1, ?)",
        );
        const now = Date.now();
    ...
    `);
      ...
      if (!imgGenCols.find((c) => c.name === "source_image_path")) {
        this.db.exec(
          "ALTER TABLE image_generations ADD COLUMN source_image_path TEXT",
        );
      }
      if (!imgGenCols.find((c) => c.name === "strength")) {
        this.db.exec("ALTER TABLE image_generations ADD COLUMN strength REAL");
      }
      const imgSessCols = this.db.pragma("table_info(image_sessions)") as {
        name: string;
      }[];
      if (!imgSessCols.find((c) => c.name === "session_type")) {
        this.db.exec(
          "ALTER TABLE image_sessions ADD COLUMN session_type TEXT DEFAULT 'generate'",
        );
      }
      ...
      try {
        ...
        this.db.exec(
          "INSERT INTO sessions (id, model_path, host, port, status, config, created_at, updated_at) VALUES ('__check__', '__check__', '0.0.0.0', 0, 'standby', '{}', 0, 0)",
        );
  ...
  getChatsByModelPath(modelPath: string): Chat[] {
    this.ensureOpen();
    const stmt = this.db.prepare(
      "SELECT * FROM chats WHERE model_path = ? ORDER BY updated_at DESC",
    );
  ...
  getChatOverrides(chatId: string): ChatOverrides | undefined {
    this.ensureOpen();
    const stmt = this.db.prepare(
      "SELECT * FROM chat_overrides WHERE chat_id = ?",
    );
    ...
    const enableThinking =
      row.enable_thinking === null || row.enable_thinking === undefined
        ? undefined // Auto
  ...
  clearChatOverrides(chatId: string): void {
    this.ensureOpen();
    const stmt = this.db.prepare(
      "DELETE FROM chat_overrides WHERE chat_id = ?",
    );
    stmt.run(chatId);
  }
  ...
  saveChatProfile(
    ...
    if (isDefault) {
      this.db
        .prepare("UPDATE chat_profiles SET is_default = 0 WHERE is_default = 1")
        .run();
    }
    this.db
      .prepare(
  ...
  getChatProfiles(): Array<{
    ...
    this.ensureOpen();
    const rows = this.db
      .prepare("SELECT * FROM chat_profiles ORDER BY name")
      .all() as any[];
    return rows.map((r) => ({
      id: r.id,
      name: r.name,
  ...
  getDefaultChatProfile(): Record<string, any> | undefined {
    this.ensureOpen();
    const row = this.db
      .prepare("SELECT overrides_json FROM chat_profiles WHERE is_default = 1")
      .get() as any;
    if (!row) return undefined;
  ...
  getSessionByModelPath(modelPath: string): Session | undefined {
    ...
    const stmt = this.db.prepare(
      "SELECT * FROM sessions WHERE model_path = ? OR model_path = ?",
    );
  ...
  updateSession(id: string, updates: Partial<Session>): void {
    ...
    if ("lastStoppedAt" in updates) {
      fields.push("last_stopped_at = ?");
      values.push(updates.lastStoppedAt ?? null);
    }
    ...
    if ("remoteUrl" in updates) {
      ...
      values.push(updates.remoteUrl ?? null);
    }
    if ("remoteApiKey" in updates) {
      fields.push("remote_api_key = ?");
      values.push(
        updates.remoteApiKey ? encryptValue(updates.remoteApiKey) : null,
      );
    }
    if ("remoteModel" in updates) {
      fields.push("remote_model = ?");
      values.push(updates.remoteModel ?? null);
    }
    if ("remoteOrganization" in updates) {
      fields.push("remote_organization = ?");
      values.push(updates.remoteOrganization ?? null);
    }
    if ("lastRequestAt" in updates) {
      fields.push("last_request_at = ?");
      values.push(updates.lastRequestAt ?? null);
    }
    if ("standbyDepth" in updates) {
      fields.push("standby_depth = ?");
      values.push(updates.standbyDepth ?? null);
    }
  ...
  private mapSessionRow(row: any): Session {
    return {
      ...
      remoteApiKey: row.remote_api_key
        ? decryptValue(row.remote_api_key)
        : undefined,
  ...
  getModelSettings(modelPath: string): Record<string, any> | undefined {
    this.ensureOpen();
    const stmt = this.db.prepare(
      "SELECT * FROM model_settings WHERE model_path = ?",
    );
  ...
  getAllModelSettings(): Record<string, any>[] {
    this.ensureOpen();
    const stmt = this.db.prepare(
      "SELECT * FROM model_settings ORDER BY model_path",
    );
  ...
  deleteModelSettings(modelPath: string): void {
    this.ensureOpen();
    const stmt = this.db.prepare(
      "DELETE FROM model_settings WHERE model_path = ?",
    );
    stmt.run(modelPath);
  }
  ...
  saveBookmark(path: string, bookmark: string): void {
    this.ensureOpen();
    const stmt = this.db.prepare(
      "INSERT OR REPLACE INTO bookmarks (path, bookmark) VALUES (?, ?)",
    );
  ...
  getBookmark(path: string): string | null {
    this.ensureOpen();
    const stmt = this.db.prepare(
      "SELECT bookmark FROM bookmarks WHERE path = ?",
    );
  ...
  getImageSessions(): ImageSession[] {
    ...
    return this.db
      ...
      .map((r: any) => ({
        id: r.id,
        modelName: r.model_name,
        sessionType: r.session_type || "generate",
        createdAt: r.created_at,
  ...
  getImageSession(id: string): ImageSession | undefined {
    this.ensureOpen();
    const r = this.db
      .prepare("SELECT * FROM image_sessions WHERE id = ?")
      .get(id) as any;
  ...
  updateImageSession(id: string, updates: Partial<ImageSession>): void {
    ...
    values.push(id);
    this.db
      .prepare(`UPDATE image_sessions SET ${fields.join(", ")} WHERE id = ?`)
      .run(...values);
  }
  ...
  addImageGeneration(gen: ImageGeneration): void {
    ...
    this.db
      ...
      .run(
        ...
        gen.seed ?? null,
        gen.strength ?? null,
        gen.elapsedSeconds ?? null,
        gen.imagePath,
        gen.sourceImagePath || null,
        gen.createdAt,
      );
  ...
  getImageGenerations(sessionId: string): ImageGeneration[] {
    ...
    return this.db
      .prepare(
        "SELECT * FROM image_generations WHERE session_id = ? ORDER BY created_at ASC",
      )
      .all(sessionId)
      .map((r: any) => ({
        id: r.id,
  ...
  getAllImageModelPaths(): ImageModelPath[] {
    this.ensureOpen();
    return this.db
      .prepare("SELECT * FROM image_model_paths ORDER BY model_id, quantize")
      .all()
      .map((row: any) => ({
        modelId: row.model_id,
        quantize: row.quantize,
  ...
  private ensureOpen(): void {
    if (this.closed) {
      console.warn(
        "[DB] Database was closed but is being accessed — reopening",
      );
      try {
