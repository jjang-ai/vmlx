...
async function detectModelFormat(
  modelPath: string,
): Promise<"mlx" | "diffusers" | "gguf" | "unknown"> {
  try {
    ...
    if (files.includes("transformer") && files.includes("text_encoder"))
      return "diffusers";
    if (files.includes("transformer") && files.includes("vae"))
      return "diffusers";
    ...
    if (hasSafetensors && hasConfig) {
      try {
        const cfg = JSON.parse(
          readFileSync(join(modelPath, "config.json"), "utf-8"),
        );
        if (cfg.model_type || cfg.architectures || cfg.quantization)
          return "mlx";
 ...
 */
export function validateImageModelCompleteness(
  modelDir: string,
  ...
  if (files.includes("transformer")) {
    try {
      ...
      hasTransformerWeights = tFiles.some((f: string) =>
  ...
  if (!hasTransformerWeights) {
    ...
    const hasRootWeights = files.some((f: string) =>
      f.endsWith(".safetensors"),
    );
    if (!hasRootWeights) {
  ...
  if (files.includes("model_index.json")) {
    ...
    if (files.includes("text_encoder")) {
      try {
        ...
        const hasTeWeights = teFiles.some((f: string) =>
          f.endsWith(".safetensors"),
        );
...
function getModelDirectories(modelType?: string): string[] {
  ...
  const builtins =
    modelType === "image" ? BUILTIN_IMAGE_PATHS : BUILTIN_MODEL_PATHS;
...
async function scanModelsInPath(
  basePath: string,
  modelType?: string,
  skipDirs?: Set<string>,
): Promise<ModelInfo[]> {
        ...
        if (size >= 1024 * 1024) {
          ...
          try {
            const configRaw = await readFile(
              join(currentPath, "config.json"),
              "utf-8",
            );
            const configJson = JSON.parse(configRaw);
            if (
              typeof configJson.quantization === "string" &&
              configJson.quantization
            ) {
          ...
          if (!quantization) {
            for (const cfgName of [
              ...
              try {
                const jangRaw = await readFile(
                  join(currentPath, cfgName),
                  "utf-8",
                );
          ...
          if (!quantization) {
            const nameMatch = id
              .toLowerCase()
              .match(/\b(4bit|8bit|3bit|6bit|fp16|bf16|fp32)\b/);
...
export function registerModelHandlers(): void {
  ...
  ipcMain.handle("models:scan", async (_, modelType?: string) => {
    ...
    const deduped = allModels.filter((_m, i) => {
    ...
    console.log(
      `[MODELS] Total models found: ${deduped.length} (${allModels.length - deduped.length} duplicates removed)`,
    );
    return deduped;
  });
  ...
  ipcMain.handle(
    "models:addDirectory",
    async (_, dirPath: string, modelType?: string) => {
  ...
  ipcMain.handle("models:detectTypes", async (_, modelPaths: string[]) => {
    ...
    for (const p of modelPaths) {
      try {
        // Diffusers / mflux image models
        if (existsSync(join(p, "model_index.json"))) {
          result[p] = "image";
          continue;
        }
        if (
          existsSync(join(p, "transformer")) &&
          existsSync(join(p, "text_encoder"))
        ) {
          result[p] = "image";
          continue;
        }
        if (existsSync(join(p, "transformer")) && existsSync(join(p, "vae"))) {
          result[p] = "image";
          continue;
        }
        ...
        if (existsSync(join(p, "config.json"))) {
          try {
            const cfg = JSON.parse(
              readFileSync(join(p, "config.json"), "utf-8"),
            );
            if (
              cfg.pipeline_tag === "text-to-image" ||
              cfg.pipeline_tag === "image-to-image"
            ) {
              result[p] = "image";
              continue;
            }
            if (cfg.model_type || cfg.architectures) {
              result[p] = "text";
              continue;
            }
          } catch {}
        ...
        if (
          existsSync(join(p, "jang_config.json")) ||
          existsSync(join(p, "jang_cfg.json")) ||
          existsSync(join(p, "jjqf_config.json")) ||
          existsSync(join(p, "mxq_config.json"))
        ) {
          result[p] = "text";
          continue;
        }
  ...
  function getDownloadDirectory(): string {
    return (
      db.getSetting(DOWNLOAD_DIR_KEY) ||
      join(homedir(), ".cache/huggingface/hub")
    );
  }
  ...
  _killActiveDownload = () => {
    for (const [, job] of activeJobs) {
      if (job.process) {
        console.log(
          `[DOWNLOADS] Killing active download on quit: ${job.repoId}`,
        );
        try {
  ...
  async function startDownloadJob(job: DownloadJob) {
    ...
    emitToRenderer("models:downloadStarted", {
      jobId: job.id,
      repoId: job.repoId,
    });
    ...
    const script = [
      ...
      "    # None enables implicit environment/cache auth; False is genuinely anonymous.",
      "    tokens = [token, False]",
      "    seen = set()",
      "    for active_endpoint in endpoints:",
      "        for active_token in tokens:",
      "            key = (active_endpoint or '', active_token is False)",
      "            if key in seen:",
      "                continue",
      "            seen.add(key)",
        ...
        try {
          ...
          if (msg.type === "init") {
            ...
            lastProgress = {
              ...
              percent: pct,
              downloaded:
                msg.downloaded_bytes > 0
                  ? formatBytes(msg.downloaded_bytes)
                  : `${msg.file_num - 1} files`,
              total:
                msg.total_bytes > 0
                  ? formatBytes(msg.total_bytes)
                  : `${msg.total_files} files`,
            ...
            if (msg.speed > 0 && progressRemaining > 0) {
              ...
              lastProgress.eta =
                secs > 3600
                  ...
                  : secs > 60
                    ? `${Math.round(secs / 60)}m ${secs % 60}s`
                    : `${secs}s`;
            }
    ...
    proc.on("close", async (code: number | null) => {
      console.log(
        `[DOWNLOADS] Process exited: ${job.repoId} code=${code} cancelled=${job.wasCancelled}`,
      );
      ...
      if (job.wasPaused) {
        ...
        if (job.imageModelName != null && job.imageQuantize != null) {
          ...
          console.log(
            `[DOWNLOADS] Stored image model path: ${job.imageModelName} q=${job.imageQuantize} → ${job.modelDir}`,
          );
        }
        emitToRenderer("models:downloadComplete", {
          jobId: job.id,
        ...
        const isGatedError =
          /40[13]|gated|access.*denied|authentication|authorized/i.test(
            errorMsg,
          );
        emitToRenderer("models:downloadError", {
          jobId: job.id,
  ...
  try {
    ...
    for (const row of pending) {
      const alreadyQueued =
        downloadQueue.some((j) => j.repoId === row.repoId) ||
            ...
            for (const model of models) {
              ...
              try {
                ...
                if (repoId && !isNaN(ts)) {
                  ...
                  const alreadyQueued =
                    downloadQueue.some((j) => j.repoId === repoId) ||
                    [...activeJobs.values()].some((j) => j.repoId === repoId);
                  if (!alreadyQueued) {
                    console.log(
                      `[DOWNLOADS] Auto-resuming interrupted download: ${repoId} → ${modelDir}`,
                    );
                    downloadQueue.push({
  ...
  cleanStaleMarkers()
    .then(() => {
      if (downloadQueue.length > 0) {
        console.log(
          `[DOWNLOADS] Found ${downloadQueue.length} interrupted download(s) to resume`,
        );
        processQueue();
      }
    })
  ...
  async function migrateExistingImageModels() {
    ...
    try {
          ...
          for (const [qStr, repoId] of Object.entries(model.repoMap)) {
            ...
            if (dirLower === repoName) {
              ...
              if (validation.complete) {
                ...
                migrated++;
                console.log(
                  `[IMAGE] Migrated existing model: ${model.id} q=${quantize} → ${fullPath}`,
                );
              }
            }
          }
      ...
      if (migrated > 0) {
        console.log(
          `[IMAGE] Migration complete: registered ${migrated} existing image model(s) in DB`,
        );
      }
    } catch (err) {
    ...
    async (
      ...
      models = models.filter(
        (m: any) =>
          m.library_name ||
          m.safetensors ||
          (m.tags && m.tags.includes("safetensors")),
      );
    ...
    async (_, collectionSlug: string) => {
      ...
      return items.map((item: any) => {
        ...
        if (item.numParameters) {
          ...
          size =
            p >= 1e9
              ? `${(p / 1e9).toFixed(1)}B params`
              : `${(p / 1e6).toFixed(0)}M params`;
        }
        return {
          id: item.id || "",
  ...
  ipcMain.handle("models:startDownload", async (_, repoId: string) => {
    ...
    for (const [, aj] of activeJobs) {
      if (aj.repoId === repoId)
        return { jobId: aj.id, status: "already_downloading" };
    }
    ...
    persistDownload(job);
    console.log(
      `[DOWNLOADS] Queued: ${repoId} (${downloadQueue.length} in queue, ${activeJobs.size} active)`,
    );
    ...
    return {
      jobId: id,
      status: activeJobs.has(id) ? "downloading" : "queued",
  ...
  ipcMain.handle("models:cancelDownload", async (_, jobId?: string) => {
    ...
    const qIdx = downloadQueue.findIndex((j) =>
  ...
  ipcMain.handle("models:pauseDownload", async (_, jobId: string) => {
    if (activeJobs.has(jobId)) {
      ...
      persistDownload(job);
      emitToRenderer("models:downloadPaused", {
        jobId: job.id,
        repoId: job.repoId,
      });
  ...
  ipcMain.handle("models:downloadModel", async (_, repoId: string) => {
    ...
    const alreadyActive = Array.from(activeJobs.values()).some(
      (j) => j.repoId === repoId,
    );
      ...
      const check = () => {
        ...
        if (done) {
          if (done.status === "complete")
            resolve({ status: "complete", path: done.modelDir });
  ...
  ipcMain.handle("models:setDownloadDir", async (_, dir: string) => {
    ...
    if (
      !userDirs.includes(normalized) &&
      !BUILTIN_MODEL_PATHS.includes(normalized)
    ) {
      userDirs.push(normalized);
  ...
  ipcMain.handle(
    ...
    async (_, modelName: string, quantize: number = 4) => {
      ...
      if (!hfToken) {
        ...
        console.log(
          `[DOWNLOADS] No HF token set. Image model ${repoId} may require authentication.`,
        );
      }
      ...
      for (const [, aj] of activeJobs) {
        if (aj.repoId === repoId)
          return { jobId: aj.id, status: "already_downloading", repoId };
      }
      const queued = downloadQueue.find((j) => j.repoId === repoId);
      ...
      persistDownload(job);
      console.log(
        `[DOWNLOADS] Queued image model: ${repoId} → ${modelDir} (${downloadQueue.length} in queue, ${activeJobs.size} active)`,
      );
