...
const api = {
  ...
  chat: {
    ...
    ensureForModel: (title: string, modelPath: string) =>
      ipcRenderer.invoke('chat:ensureForModel', title, modelPath),
  ...
  developer: {
    ...
    getBufferedLogs: () => ipcRenderer.invoke('developer:getBufferedLogs') as Promise<{
      lines: string[]
      running: boolean
  ...
  image: {
    ...
    isGenerating: () => ipcRenderer.invoke('image:isGenerating') as Promise<{ generating: boolean; cancelling: boolean; startTime: number | null; sessionId: string | null }>,
  ...
  sessions: {
    ...
    getLoadProgress: () => ipcRenderer.invoke('sessions:getLoadProgress'),
