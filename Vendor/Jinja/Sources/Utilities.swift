...
// vMLX §225 (2026-04-21): Python-semantics fix for negative-step slicing.
// Upstream swift-jinja 1.3.0 defaulted start=0 / stop=arrayCount regardless
// of step sign, which made `arr[::-1]` (reverse) return an empty array.
// That broke every Qwen3.5 / Qwen3.6 multi-turn chat_template render
// (template scans `messages[::-1]` to find the last user query, raises
// "No user query found in messages." when the scan returns empty), silently
// falling back to ChatML — which has no `<think>` / `enable_thinking`
// support. Python semantics for `arr[start:stop:step]`:
//   step > 0: start defaults 0, stop defaults len
//   step < 0: start defaults len-1, stop defaults past index 0 (None)
func slice<T>(_ array: [T], start: Int? = nil, stop: Int? = nil, step: Int? = 1) -> [T] {
    let arrayCount = array.count
    let step = step ?? 1
    var slicedArray = [T]()
    guard step != 0, arrayCount > 0 else { return slicedArray }

    if step > 0 {
        let startValue = start ?? 0
        let stopValue = stop ?? arrayCount
        let startIndex = startValue < 0
            ? max(arrayCount + startValue, 0)
            : min(startValue, arrayCount)
        let stopIndex = stopValue < 0
            ? max(arrayCount + stopValue, 0)
            : min(stopValue, arrayCount)
        for i in stride(from: startIndex, to: stopIndex, by: step) {
            slicedArray.append(array[i])
        }
    } else {
        // Reverse step: Python defaults start=len-1, stop past index 0.
        let startValueRaw = start ?? (arrayCount - 1)
        let startIndex = startValueRaw < 0
            ? max(arrayCount + startValueRaw, -1)
            : min(startValueRaw, arrayCount - 1)

        // `stop` is exclusive. With `nil`, walk through index 0 inclusive.
        // `stride(through:)` includes the endpoint, so the inclusive
        // boundary is `stopExclusive + 1` (one step past the exclusive).
        let stopIndexInclusive: Int
        if let stop {
            let stopClamped = stop < 0
                ? max(arrayCount + stop, -1)
                : min(stop, arrayCount - 1)
            stopIndexInclusive = stopClamped + 1
        } else {
            stopIndexInclusive = 0
        }

        if startIndex >= stopIndexInclusive {
            for i in stride(from: startIndex, through: stopIndexInclusive, by: step) {
                slicedArray.append(array[i])
            }
        }
    }
