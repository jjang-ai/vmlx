...
def test_current_regression_suite_refreshes_release_regression_manifest(monkeypatch, tmp_path):
    ...
    assert any(
        ...
        and "build/current-dsv4-route-mode-code-exactness-memory-preflight-20260601-post-wrapped-epipe.json"
        in cmd
        for name, cmd in seen_steps
    )
    assert any(
        ...
        and "build/current-real-ui-dsv4-memory-preflight-20260601-post-wrapped-epipe.json"
        in cmd
        for name, cmd in seen_steps
    )
    assert any(
        name == "release_regression_manifest"
        and "build/current-release-regression-manifest-20260601-cache-ipc-installed-refresh.json"
        in cmd
        for name, cmd in seen_steps
    )
    assert not any(
...
def test_current_regression_suite_refreshes_current_packaged_integrity_artifact(
    ...
    assert any(
        ...
        and "build/current-api-surface-contract-20260601-cache-ipc-epipe-refresh.json"
        in cmd
        for name, cmd in seen_steps
    )
    assert not any(
