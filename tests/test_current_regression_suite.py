...
def test_current_regression_suite_refreshes_release_regression_manifest(monkeypatch, tmp_path):
    ...
    assert any(
        name == "release_regression_manifest"
        and "build/current-release-regression-manifest-after-mimo-audio-expanded-token-l2-restart-20260608.json"
        in cmd
        for name, cmd in seen_steps
    )
    assert not any(
...
def test_current_regression_suite_runs_full_release_objective_checklist(
    ...
    assert any(
        ...
        and "current-full-release-objective-checklist-after-mimo-audio-expanded-token-l2-restart-20260608.json"
        in " ".join(cmd)
        for _name, cmd in seen_steps
    )
...
def test_current_regression_suite_allows_open_full_release_objective_checklist(
    ...
    path = tmp_path / "build/current-full-release-objective-checklist-after-mimo-audio-expanded-token-l2-restart-20260608.json"
