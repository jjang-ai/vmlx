...
def test_current_regression_suite_refreshes_release_regression_manifest(monkeypatch, tmp_path):
    ...
    assert any(
        ...
        and "build/current-dsv4-route-mode-code-exactness-memory-preflight-20260601-post-public-issue-audit.json"
        in cmd
        for name, cmd in seen_steps
    )
    assert any(
        ...
        and "build/current-real-ui-dsv4-memory-preflight-20260601-post-public-issue-audit.json"
        in cmd
        for name, cmd in seen_steps
    )
    assert any(
