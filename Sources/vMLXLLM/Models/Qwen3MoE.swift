...
class Qwen3MoeDecoderLayer: Module {
    ...
    // `var` so Flash MoE Phase 2b can swap in a `FlashMoEBlock` at
    // load time. See the `FlashMoELayer` conformance at the bottom
    // of this file.
    fileprivate var mlp: UnaryLayer
...
//
// Reference implementation of the `FlashMoEReplaceable` /
// `FlashMoELayer` contracts for the text-path SwitchGLU layout.
// At load time, `FlashMoE.apply(to:loader:)` walks `flashMoELayers`
// and for each MoE layer builds a `FlashMoEBlock` and calls
// `replaceMoEBlock(with:)`. The layer captures its original gate
// (softmax + argPartition + norm) into the block's `router` closure
// and installs the block as the new `mlp` child. The original
// `Qwen3MoESparseMoeBlock` is dropped so its stacked expert
// projection weights become eligible for GC, freeing the memory
// the slot bank uses to stream experts from disk.

extension Qwen3MoEModel: FlashMoEReplaceable {
    public var flashMoELayers: [FlashMoELayer] {
        model.layers
    }
}

extension Qwen3MoeDecoderLayer: FlashMoELayer {
    public var flashMoELayout: FlashMoELayout {
        // MoE layers use the sparse block; dense (mlp_only_layers)
        // stay as Qwen3MoEMLP and are skipped by the traversal.
        (mlp is Qwen3MoESparseMoeBlock) ? .textPathSwitchGLU : .none
    }

    public func replaceMoEBlock(with block: FlashMoEBlock) throws {
        guard let sparse = mlp as? Qwen3MoESparseMoeBlock else {
            return  // dense layer, skipped
        }
        // Capture the original gate + routing policy. The closure
        // runs the same math as `Qwen3MoESparseMoeBlock.callAsFunction`
        // but stops at `(indices, scores)` so FlashMoEBlock can do
        // the per-expert matmul itself via the slot bank.
        let gate = sparse.gate
        let topK = sparse.topK
        let normTopkProb = sparse.normTopkProb
        block.topK = topK
        block.router = { x in
            let gates = gate(x)
            let softGates = MLX.softmax(gates, axis: -1, precise: true)
            let inds = MLX.argPartition(-gates, kth: topK - 1, axis: -1)[
                .ellipsis, ..<topK]
            var scores = MLX.takeAlong(softGates, inds, axis: -1)
            if normTopkProb {
                scores = scores / MLX.sum(scores, axis: -1, keepDims: true)
        ...
        // Install. Direct assignment is enough here because the
        // decoder layer's `mlp` is a plain `var` (non-@ModuleInfo)
        // so the Module child walk uses runtime reflection and
        // will pick up the new value on the next parameters() call.
        // The old sparse block's weights are released when ARC
