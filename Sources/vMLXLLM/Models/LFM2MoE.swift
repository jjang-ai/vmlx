...
class LFM2MoEDecoderLayer: Module {
    ...
    /// Flash MoE Phase 2b shim. Only installed on sparse layers
    /// (`!usesDenseFeedForward`). When set, the `callAsFunction`
    /// routes the feed-forward step through the slot-bank shim.
    ...
    func callAsFunction(
        ...
        let h = x + r
        let ffnInput = ffnNorm(h)
        let out = flashMoeShim.map { $0(ffnInput) } ?? feedForward(ffnInput)
        return h + out
    }
}
...
extension LFM2MoEModel: FlashMoEReplaceable {
    public var flashMoELayers: [FlashMoELayer] {
        model.layers
    }
}

extension LFM2MoEDecoderLayer: FlashMoELayer {
    public var flashMoELayout: FlashMoELayout {
        // Dense layers and attention-only layers don't route experts.
        usesDenseFeedForward ? .none : .textPathSwitchGLU
    }

    public func replaceMoEBlock(with block: FlashMoEBlock) throws {
        guard !usesDenseFeedForward,
              let sparse = feedForward as? Lfm2MoeSparseMoeBlock
        else { return }
        let gate = sparse.gate
        let topK = sparse.topK
        let normTopKProb = sparse.normTopKProb
        let useExpertBias = sparse.useExpertBias
        let expertBias = sparse.expertBias
        block.topK = topK
        block.router = { x in
            var gates = MLX.softmax(gate(x), axis: -1, precise: true)
            if useExpertBias, let bias = expertBias {
                gates = gates + bias
            }
            let inds = argPartition(-gates, kth: topK - 1, axis: -1)[
                .ellipsis, ..<topK]
            var scores = takeAlong(gates, inds, axis: -1)
            if normTopKProb {
                let denom = scores.sum(axis: -1, keepDims: true)
