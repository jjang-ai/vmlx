...
class OlmoETransformerBlock: Module {
    ...
    func callAsFunction(
        ...
        let moeInput = postAttentionLayerNorm(x)
        x = x + (flashMoeShim.map { $0(moeInput) } ?? mlp(moeInput))
        return x
    }
}
...
extension OlmoEModel: FlashMoEReplaceable {
    public var flashMoELayers: [FlashMoELayer] {
        model.layers
    }
}

extension OlmoETransformerBlock: FlashMoELayer {
