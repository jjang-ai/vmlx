...
class BailingMoeSparseMoeBlock: Module, UnaryLayer {
    ...
    /// Flash MoE Phase 2b shim. Replaces the routed-experts matmul
...
//
// BailingMoe has a grouped-top-K gate with sigmoid + expert bias +
// group zero-masking. We capture the `BailingMoeGate` reference and
// reuse its `groupSelect` directly — it already returns the
// (indices, weights) tuple the FlashMoE router expects. Shared
// experts survive via the `flashMoeShim` branch inside
// `BailingMoeSparseMoeBlock`.

extension BailingMoeModel: FlashMoEReplaceable {
    public var flashMoELayers: [FlashMoELayer] {
        model.layers
    }
}

extension BailingMoeTransformerBlock: FlashMoELayer {
    public var flashMoELayout: FlashMoELayout {
        (mlp is BailingMoeSparseMoeBlock) ? .textPathSwitchGLU : .none
    }

    public func replaceMoEBlock(with block: FlashMoEBlock) throws {
        guard let sparse = mlp as? BailingMoeSparseMoeBlock else { return }
        let gate = sparse.gate
        block.topK = gate.topK
        block.router = { x in
            let out = gate.groupSelect(x)
            return (indices: out.inds, scores: out.scores)
        }
        sparse.flashMoeShim = block
    }
}
