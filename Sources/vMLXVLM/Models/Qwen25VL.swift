...
public class Qwen25VL: Module, VLMModel, KVCacheDimensionProvider {
        ...
        -> PrepareResult
    {
        ...
        // Chunked prefill — VLM half of vmlx #50/#51. Long image prompts
        // blow through the Metal single-buffer cap when pushed through
        // the language model in one shot. `chunkedPrefillEmbedding`
        // slices the embedding along axis 1 and flushes between chunks,
        // mirroring `LLMModel.prepare`'s text-only path.
        let result = chunkedPrefillEmbedding(
            inputEmbedding: inputEmbeddings,
            cache: cache,
            prefillStepSize: windowSize ?? 512
        ) { chunk in
            languageModel(nil, cache: cache, inputEmbedding: chunk)
        }
