...
// vMLX adaptation: separate sparse QK with materialized input-dtype scores.
// Preserve the upstream MMA reduction order and defer softmax/PV to MLX.
template <
    typename T,
    int BK,
    int DC,
...
qwen4_qsa_sparse_gqa_scores(
  ...
  const int ktile = int(tid.z);
  {
    ...
    // Store only selected scores directly into stock head/query/token
    // layout. A preceding fill dispatch initializes all masked positions.
    using stile_t = decltype(Stile);
    STEEL_PRAGMA_UNROLL
    for (short i = 0; i < stile_t::kTileRows; ++i) {
      const int head = tm + sm + i * stile_t::kFragRows;
      if (head < GQA) {
        STEEL_PRAGMA_UNROLL
        for (short j = 0; j < stile_t::kTileCols; ++j) {
          STEEL_PRAGMA_UNROLL
          for (short e = 0; e < stile_t::MMAFrag_t::kElemCols; ++e) {
            const int token = selected[sn + j * stile_t::kFragCols + e];
            if (token >= 0) {
              const size_t at = (size_t(kv_head * GQA + head) * params->qL + q_pos)
                                  * params->kL + token;
              O[at] = T(Stile.frag_at(i, j)[e]);
            }
          }
        }
      }
